    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                formData.append('_subject', `Новое сообщение от ${name}`);
                formData.append('_to', 'Anton_lavren@mail.ru');
                formData.append('_replyto', email);
                formData.append('_captcha', 'false');

                const response = await fetch('https://formsubmit.co/ajax/Anton_lavren@mail.ru', {
                    method: 'POST',
                    body: formData
                });
